...
def train(net, trainloader, epochs, lr, device):
    ...
    for _ in range(epochs):
        for batch in trainloader:
            ...
            logits = net(x)
            loss = crit(logits, y)   # média do batch
            loss.backward()
            opt.step()

            # métrica correta: média por exemplo
            loss_sum += loss.item() * y.size(0)  # mean × batch_size
            total    += y.size(0)
    # avg_trainloss = running_loss / len(trainloader)
    avg_trainloss = loss_sum / max(total, 1)
...
def test(net, testloader, device):
    ...
    with torch.no_grad():
        for batch in testloader:
            ...
            logits = net(x)
            loss_sum += criterion(logits, y).item()     # soma por amostra
    ...
    loss = loss_sum / max(total, 1)                    # média por exemplo
